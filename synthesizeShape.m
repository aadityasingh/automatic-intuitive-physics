...
function synthesizeShape(shape, original, replace, original_num, replace_num, display)

objects = ["Sphere", "Cube", "Cyl"];
...
peak_params(:, :, 1) = [0.03 0.001; 0.03 0.004; 0.08 0.01];
peak_params(:, :, 2) = [0.03 0.001; 0.03 0.004; 0.08 0.01];
peak_params(:, :, 3) = [0.03 0.001; 0.03 0.004; 0.08 0.01];

original_audio = audioread(char(objects(shape)+infix+surfaces(original)+original_num+suffix));
replace_audio = audioread(char(objects(replace_shape)+infix+surfaces(replace)+replace_num+suffix));

% figure(2);
% findpeaks(original_audio(:, 3), 44100, 'MinPeakDistance', peak_params(original, 1, shape), 'MinPeakHeight', peak_params(original, 2, shape))
% figure(3);
% findpeaks(replace_audio(:, 3), 44100, 'MinPeakDistance', peak_params(replace, 1, 1), 'MinPeakHeight', peak_params(replace, 2, 1))
[pks3, locs3, widths3] = findpeaks(original_audio(:, 3), 44100, 'MinPeakDistance', peak_params(original, 1, shape), 'MinPeakHeight', peak_params(original, 2, shape));
[pks3_rep, locs3_rep, widths3_rep] = findpeaks(replace_audio(:, 3), 44100, 'MinPeakDistance', peak_params(replace, 1, 1), 'MinPeakHeight', peak_params(replace, 2, 1));
...
replace_peak_len = int32(fs*(locs3_rep(2) - locs3_rep(1))-25);
...
disp(objects(shape)+infix+surfaces(original)+original_num)
disp("Dubbed"+surfaces(replace)+replace_num)
audiowrite(char(objects(shape)+infix+surfaces(original)+original_num+"Dubbed"+surfaces(replace)+replace_num+suffix), new_audio, fs);
